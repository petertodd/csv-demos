...
from bitcoin.core.script import (
        OP_NOP3, OP_DROP, OP_CHECKSIG,
        CScript,
        SignatureHash, SIGHASH_ALL,
)
from bitcoin.wallet import P2SHBitcoinAddress, CBitcoinSecret, CBitcoinAddress

parser = argparse.ArgumentParser(description="hodl your bitcoins with CHECKSEQUENCEVERIFY")
...
def hodl_redeemScript(privkey, nLockTime):
    return CScript([nLockTime, OP_NOP3, OP_DROP,
...
parser_create.add_argument('-w',
        metavar='WALLET',
        default='wallet.csv-hodl',
...
parser_create.add_argument('-t', '--time',
        ...
        action='store',
        type=int,
        help='Hodl for n seconds')
parser_create.add_argument('-b', '--blocks',
        metavar='BLOCKS',
        action='store',
        type=int,
        help='Hodl for n blocks')
